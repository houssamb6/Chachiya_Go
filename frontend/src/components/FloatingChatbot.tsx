...
const FloatingChatbot = () => {
    ...
    if (ctxSessionId) {
      if (!historyLoaded && messages.length === 0) {
        ...
        try {
          ...
          const fromYasmine = historyToMessages(session.yasmine_history ?? []);
          const fromQa = (session.qa_history ?? []).length > 0
            ? historyToMessages(session.qa_history)
            : [];
          setMessages([...fromYasmine, ...fromQa]);
          if (session.phase === "quiz" || session.phase === "qa") setQuizLoaded(true);
        } catch {
  ...
  const handleSend = async (text?: string) => {
    ...
    const handleResponse = (r: ChouchaneResponse) => {
      ...
      // If we just moved to quiz, load the question once
      if (r.phase === "quiz") {
        if (!r.quiz_question) {
          quizStart(sid).then((qr) => {
            setCtxPhase(qr.phase);
            appendAssistant(setMessages, qr.reply);
            setQuizLoaded(true);
          }).catch((err) =>
            appendError(setMessages, err instanceof Error ? err.message : String(err))
          ).finally(() => setLoading(false));
    ...
    try {
      if (phase === "yasmine") {
        ...
        if (!quizLoaded) {
          const r = await quizStart(sid);
          handleResponse(r);
        } else {
          const r = await quizAnswer(sid, msg);
          handleResponse(r);
        }
      } else {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-x-2 bottom-2 top-16 z-[60] max-w-md mx-auto flex flex-col rounded-2xl border border-border bg-background shadow-elevated overflow-hidden"
          >
