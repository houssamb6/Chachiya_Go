 ...
 * Talks to the Tunisia Tourism AI backend: Chouchene → Quiz → Q&A.
 */

const BASE_URL = import.meta.env.VITE_CHOUCHANE_API_URL ?? "http://localhost:8000";

export type ChouchanePhase = "yasmine" | "quiz" | "qa";
...
/** Phase 2: Load quiz question for the chosen destination. Call once after phase becomes quiz. */
export async function quizStart(sessionId: string): Promise<ChouchaneResponse> {
  return request<ChouchaneResponse>("/quiz/start", {
    method: "POST",
    body: { session_id: sessionId, message: "" },
  });
}

/** Phase 2: Submit quiz answer or request hint. */
export async function quizAnswer(
  sessionId: string,
  message: string
): Promise<ChouchaneResponse> {
  return request<ChouchaneResponse>("/quiz/answer", {
    method: "POST",
    body: { session_id: sessionId, message },
  });
}
...
export interface PlaceFromApi {
  ...
  insider_tip: string;
  season: string;
  quiz?: { question: string; hints: string[]; answer: string };
}
