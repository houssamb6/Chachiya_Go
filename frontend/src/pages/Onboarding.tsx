...
const Onboarding = () => {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      // Chouchene finished: phase moved to quiz (user chose a place) OR agent sent the closing message
      const closingInReply = /your personalized (Tunisian )?adventure is ready/i.test(res.reply);
      if (res.phase === "quiz" || closingInReply) {
        if (!closingInReply) {
          msgId.current++;
          setMessages((prev) => [
            ...prev,
        ...
        <div className="flex flex-col h-screen">
          ...
          <div className="px-5 pt-[max(1rem,env(safe-area-inset-top))] pb-3 border-b border-border bg-card/80 backdrop-blur-sm">
            ...
            <div className="flex gap-1.5 mt-3">
              ...
              />
              <div className={`h-1 flex-1 rounded-full transition-all duration-500 ${phase === "quiz" ? "bg-primary" : "bg-border"}`} />
            </div>
          </div>
